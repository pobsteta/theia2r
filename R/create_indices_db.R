...
create_indices_db <- function(xslt_path = NA,
  ...
  if (is.na(json_path)) {
    json_path <- file.path(system.file("extdata",package="theia2r"),"indices.json")
  }
  if (system.file("extdata","indices.json", package="theia2r") == json_path) {
    if (force == FALSE) {
  ...
  # set XSLT path
  if (is.na(xslt_path)) {
    xslt_path <- file.path(system.file(package="theia2r"),"extdata","xslt")
  }
