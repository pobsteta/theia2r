...
safe_shortname <- function(prod_name, prod_type = NULL, ext = NULL, res = "10m", tiles = NA, force_tiles = FALSE, full.name = TRUE, set.seed = NA, multiple_names = FALSE, 
    abort = FALSE) {
    ...
    if (any(sapply(s2_metadata[mandatory_metadata], is.null))) {
        print_message(type = "error", "Some of the required elements ('", paste(mandatory_metadata[sapply(s2_metadata[mandatory_metadata], is.null)], collapse = "' '"), 
    ...
    # Define tiles ID to use if tile_id (= tile as retrieved from product name) exists, consider it; otherwise, use tiles (existing tile list)
    ...
    # if sel_tiles is missing or multiple tiles must be reduced to one, compute a random tile ID
    if (is.null(sel_tiles) | length(sel_tiles) > 1 & multiple_names == FALSE) {
        ...
        if (length(s2_metadata$utm) == 1) {
            ...
            sel_tiles <- paste0(s2_metadata$utm, str_pad(sample(1000, 1) - 1, 3, "left", "0"))
        } else {
            sel_tiles <- paste0(paste(LETTERS[sample(26, 2, replace = TRUE)], collapse = ""), str_pad(sample(1000, 1) - 1, 3, "left", "0"))
        }
    ...
    # check prod_type
    prod_type_accepted_values <- list(`1C` = c("TOA"), `2A` = c("BOA", "TCI", "AOT", "WVP", "SCL", "CLD", "SNW", "VIS"))
    message_type <- ifelse(abort == TRUE, "error", "warning")
    if (!prod_type %in% c(prod_type_accepted_values[[s2_metadata$level]], "xxx")) {
        print_message(type = message_type, "\"prod_type\" value is not recognised (for level ", s2_metadata$level, " accepted values are '", paste(prod_type_accepted_values[[s2_metadata$level]], 
            collapse = "', '"), "'; use 'xxx' for a generic string in order not to show this warning).")
    }
    
    # check res
    if (!res %in% c("10m", "20m", "60m")) {
        print_message(type = message_type, "\"res\" value is not recognised (accepted values are '10m', '20m' ", "and '60m').")
    }
    
    # compose name
    short_name <- with(s2_metadata, paste0("S", mission, level, "_", strftime(s2_metadata$sensing_datetime, "%Y%m%d"), "_", id_orbit, "_", sel_tiles, "_", prod_type, 
        "_", substr(res, 1, 2)))
    
    # check name length
    if (any(nchar(short_name) != 31)) {
        print_message(type = "warning", "Length of shortname is wrong (probably \"prod_type\" or \"res\" ", "has wrong length).")
    }
