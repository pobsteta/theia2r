...
install_aria2 <- function(aria2_dir = system.file(package = "theia2r"), force = FALSE) {
    ...
    if (Sys.info()["sysname"] != "Windows") {
        print_message(type = "warning", "This function is only for Windows.")
    ...
    if (force != TRUE & !is.null(binpaths$aria2c)) {
        ...
        if (file.exists(aria2_bin)) {
            print_message(type = "message", "aria2 is already installed; to overwrite, set force = TRUE.")
            return(invisible(NULL))
        }
    }
    
    # Download aria2
    aria2_url <- paste0("https://github.com/aria2/aria2/releases/download/release-", aria2_ver, "/aria2-", aria2_ver, if (Sys.info()["machine"] == "x86-64") {
        "-win-64bit"
    } else {
        "-win-32bit"
    }, "-build1.zip")
    ...
    system(paste(binpaths$wget, aria2_url, "-O", aria2_zip), intern = Sys.info()["sysname"] == "Windows")
    if (file.exists(aria2_zip)) {
        aria2_filelist <- unzip(zipfile = aria2_zip, list = TRUE)
        aria2_filename <- aria2_filelist$Name[grep(paste0(basename(aria2_path), "$"), aria2_filelist$Name)]
        unzip(zipfile = aria2_zip, files = aria2_filename, exdir = dirname(aria2_path), junkpaths = TRUE, unzip = "internal")
        unlink(aria2_zip)
    }
    ...
    # # add to the path system(paste0('setx PATH '',binpaths$aria2c,'''), intern=TRUE)
