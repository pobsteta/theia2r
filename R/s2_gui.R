...
# For now, value "theia2r" is used when s2_gui() is launched from
# theia2r(), and is used to change save button label.
.s2_gui <- function(param_list = NULL,
                    par_fun = "parent",
                    thunderforest_api = NA) {
  ...
  s2_gui.ui <- dashboardPage(
    ...
    sidebar = dashboardSidebar(
      ...
      div(
        ...
        a(
          href='https://pobsteta.github.io/theia2r',
          target = "_blank",
          uiOutput("img_logo")
        )
      ),
  ...
  s2_gui.server <- function(input, output, session) {
    ...
    observeEvent(input$help_downloader, {
      showModal(modalDialog(
        ...
        p(HTML(
          ...
          "using the function <code>check_theia2r_deps()</code> (graphically) or",
        ...
        p(HTML(
          ...
          "or in Windows using the function <code>check_theia2r_deps()</code>",
          "(graphically) or <code>install_aria2()</code> (from commandline)."
        )),
        p(HTML(
          "This selector is active only in aria2 was already installed and",
          "recognised (to recognise it, launch <code>check_theia2r_deps()</code>)."
        )),
        easyClose = TRUE,
        footer = NULL
      ))
    ...
    observeEvent(input$alert_leaflet, {
      showModal(modalDialog(
        ...
        p(HTML(
          ...
          "(as an argument of theia2r() function, or with a parameter list)",
    ...
    observeEvent(input$import_param_deactivated, {
      showModal(modalDialog(
        ...
        p(HTML(
          ...
          "To import a JSON option file, launch <tt>theia2r()</tt>",
          "function with the path of the file as argument:"
        )),
        p(HTML(
          "<tt>theia2r(param_list = \"/path/of/the/file.json\")</tt>"
        )),
        easyClose = TRUE,
        footer = NULL
      ))
    ...
    create_return_list <- function() {
      ...
      rl$pkg_version <- packageVersion("theia2r") %>% as.character()
    ...
    #   filename = function() {
    #     paste0("theia2r_", strftime(Sys.Date(),"%Y%m%d"), ".json")
    #   },
