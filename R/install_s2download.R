...
install_s2download <- function(inst_path = NA) {
    .install_s2download(inst_path = inst_path, interactive = TRUE)
}

.install_s2download <- function(inst_path = NA, interactive = TRUE) {
    ...
    if (is.na(inst_path)) {
        inst_path <- file.path(system.file(package = "theia2r"), "s2download")
    }
    if (file.exists(inst_path) & !file.info(inst_path)$isdir) {
        print_message(type = "error", inst_path, " already exists and it is a file; please provide a different value (or leave blank).")
    }
    if (length(list.files(inst_path)) > 0) {
        if (interactive & interactive()) {
            print_message(type = "waiting", paste0(inst_path, " already exists and will be erased: ENTER to proceed or ESC to cancel..."))
        } else {
            print_message(type = "warning", inst_path, " already exists and will be erased.")
        }
    ...
    unzip(zipfile = s2download_zippath, exdir = dirname(inst_path), unzip = "internal") %>% suppressWarnings()
    ...
    install_s2download_dependencies <- tryCatch(import_from_path("install_dependencies", inst_path, convert = FALSE), error = function(e) {
        e
    })
    if (is(install_s2download_dependencies, "error")) {
        install_s2download_dependencies <- import_from_path("install_dependencies", normalize_path(paste0(inst_path, "/")), convert = FALSE)
    }
    
    # clone dependent repositories
    install_s2download_dependencies$download_repo(c("ranghetti", "Sentinel-download"))
    
    # TODO check on errors (bot in python some of them does not appear as errors) and message in case all run ok.
