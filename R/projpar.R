...
projpar <- function(proj4string, par, abort = FALSE) {
    ...
    crs_check <- tryCatch(st_crs2(proj4string), error = function(e) {
        st_crs(NA)
    })
    ...
    proj4_wkt <- st_as_text(crs_check) %>% r_to_py() %>% py$osr$SpatialReference()
    proj4_par <- proj4_wkt$GetAttrValue(par) %>% py_to_r()
