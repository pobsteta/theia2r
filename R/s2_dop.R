...
s2_dop <- function(s2_orbits = 1:143, timewindow = "10 days", mission = c("2A", "2B")) {
    ...
    if (length(s2_orbits) == 0 | length(timewindow) == 0 | length(mission) == 0) {
        return(data.table(date = as.Date(character(0)), mission = character(0), orbit = character(0), stringsAsFactors = FALSE))
    }
    ...
    if (!all(grepl("^2[AB]$", mission))) {
        print_message(type = "error", "Parameter 'mission' cannot contain values different from \"2A\" and/or \"2B\".")
    }
    
    ## Check timewindow check that it does not contain NAs
    if (anyNA(timewindow)) {
        print_message(type = "error", "Parameter 'timewindow' cannot contain NA values.")
    }
    
    # Check to be 1 or 2 length
    if (length(timewindow) > 2) {
        print_message(type = type, "Parameter 'timewindow' must be of length 1 or 2.")
    }
    ...
    if (is(timewindow, "character")) {
        ...
        if (is.character(try_date)) {
            ...
            if (any(!grepl("^((\\-?[0-9]+)|(this)|(days?)|(weeks?)|(months?)|(years?))$", tolower(timewindow))) | length(timewindow) > 2) {
                print_message(type = "error", "Parameter 'timewindow' is not a recognised string.")
            }
            ...
            timewindow_start <- switch(tolower(gsub("s$", "", timewindow[2])), day = Sys.Date(), week = as.Date(cut(Sys.Date(), "week")), month = as.Date(strftime(Sys.Date(), 
                "%Y-%m-01")), year = as.Date(strftime(Sys.Date(), "%Y-01-01")))
            timewindow_all <- if (as.integer(timewindow[1]) > 0) {
                seq(timewindow_start, length = as.integer(timewindow[1]) + 1, by = timewindow[2])
            } else {
                sort(c(seq(timewindow_start, length = 2, by = timewindow[2])[2], seq(timewindow_start, length = -as.integer(timewindow[1]), by = paste("-1", timewindow[2]))))
            }
    ...
    if (!is(timewindow, "Date")) {
        print_message(type = type, "Parameter 'timewindow' is not in a recognised format.")
    }
    ...
    s2a_orbits <- lapply(s2a_dates, function(d) {
        sel_dop_dt[doybase == as.integer(d)%%10, orbit]
    })
    s2b_orbits <- lapply(s2b_dates, function(d) {
        sel_dop_dt[doybase == (as.integer(d) + 5)%%10, orbit]
    })
    s2_missions <- c(rep("2A", length(s2a_dates)), rep("2B", length(s2b_dates)))
    s2_data <- rbindlist(lapply(seq_along(s2_missions), function(i) {
        expand.grid(date = c(s2a_dates, s2b_dates)[i], mission = s2_missions[i], orbit = c(s2a_orbits, s2b_orbits)[[i]], stringsAsFactors = FALSE)
    }))
    ...
    if (nrow(s2_data) > 0) {
        ...
        s2_data <- s2_data[mission == "2A" & date >= "2015-06-27" | mission == "2B" & date >= "2017-06-29"]
        return(s2_data)
    } else {
        return(data.table(date = as.Date(character(0)), mission = character(0), orbit = character(0), stringsAsFactors = FALSE))
    }
