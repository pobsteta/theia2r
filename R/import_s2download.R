...
import_s2download <- function(with_aria2 = TRUE, ...) {
    ...
    py <- init_python()
    # load s2download
    s2download <- tryCatch(reticulate::import_from_path("s2download", s2download_path, ...), error = print)
    if (is(s2download, "error")) {
        s2download <- reticulate::import_from_path("s2download", paste0(normalize_path(s2download_path), "/"), ...)
    }
