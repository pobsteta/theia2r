...
install_wget <- function(wget_dir = system.file(package = "theia2r"), force = FALSE) {
    ...
    if (force != TRUE & !is.null(binpaths$wget)) {
        ...
        if (file.exists(wget_bin)) {
            print_message(type = "message", "wget is already installed; to overwrite, set force = TRUE.")
            return(invisible(NULL))
        }
    }
    
    # Download wget
    wget_url <- paste0("https://eternallybored.org/misc/wget/releases/wget-", wget_ver, if (Sys.info()["machine"] == "x86-64") {
        "-win64"
    } else {
        "-win32"
    }, ".zip")
    ...
    if (file.exists(wget_zip)) {
        unzip(zipfile = wget_zip, files = basename(wget_path), exdir = dirname(wget_path), unzip = "internal")
        unlink(wget_zip)
    }
    ...
    # # add to the path system(paste0('setx PATH '',binpaths$wget,'''), intern=TRUE)
