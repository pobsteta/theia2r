...
s2_download <- function(s2_prodlist = NULL, downloader = "wget", apihub = NA, tile = NULL, outdir = ".") {
    ...
    if (!downloader %in% c("wget", "aria2")) {
        print_message(type = "warning", "Downloader \"", downloader, "\" not recognised ", "(wget will be used)")
        downloader <- "wget"
    }
    ...
    for (i in seq_len(length(s2_prodlist))) {
        ...
        # download archive for compactname products and for oldname on Windows, untile #107 will be solved
        if (safe_getMetadata(filename, "nameinfo")$version == "compact" | Sys.info()["sysname"] == "Windows") {
        ...
        print_message(type = "message", date = TRUE, "Checking product ", i, " of ", length(s2_prodlist), " (", filename, ")...")
        
        trace_function(s2download$download_s2product, filename = filename, link = link, downloader = downloader, apihub = apihub, tile = py_tile, no_download = FALSE, 
            write_dir = outdir, file_list = NULL, downloader_path = dirname(binpaths[[if (downloader == "aria2") {
                "aria2c"
            } else {
                "wget"
            }]]), trace_funname = "s2download", trace_files = file.path(outdir, c(filename, paste0(filename, ".zip"))))
        # s2download$download_s2product( filename = filename, link = link, downloader = downloader, apihub = apihub, tile = py_tile, no_download = FALSE, write_dir = outdir,
        # file_list = NULL, downloader_path = dirname(binpaths[[if (downloader=='aria2') {'aria2c'} else {'wget'}]]) )
